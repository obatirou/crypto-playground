...
contract AES {
    ...
    function subBytes(AESState memory state) public pure returns (AESState memory) {
        state.col0 = subWord(state.col0);
        state.col1 = subWord(state.col1);
        state.col2 = subWord(state.col2);
        state.col3 = subWord(state.col3);
        return state;
    }
    ...
    function shiftRows(AESState memory state) public pure returns (AESState memory) {
        ...
        rows[0] = _bytes1ToBytes4(state.col0[0], state.col1[0], state.col2[0], state.col3[0]);
        rows[1] = _bytes1ToBytes4(state.col1[1], state.col2[1], state.col3[1], state.col0[1]);
        rows[2] = _bytes1ToBytes4(state.col2[2], state.col3[2], state.col0[2], state.col1[2]);
        rows[3] = _bytes1ToBytes4(state.col3[3], state.col0[3], state.col1[3], state.col2[3]);
        state.col0 = _bytes1ToBytes4(rows[0][0], rows[1][0], rows[2][0], rows[3][0]);
        state.col1 = _bytes1ToBytes4(rows[0][1], rows[1][1], rows[2][1], rows[3][1]);
        state.col2 = _bytes1ToBytes4(rows[0][2], rows[1][2], rows[2][2], rows[3][2]);
        state.col3 = _bytes1ToBytes4(rows[0][3], rows[1][3], rows[2][3], rows[3][3]);
        return state;
    }
    ...
    function mixColumns(AESState memory state) public pure returns (AESState memory) {
        state.col0 = _mixColumn(state.col0);
        state.col1 = _mixColumn(state.col1);
        state.col2 = _mixColumn(state.col2);
        state.col3 = _mixColumn(state.col3);
        return state;
    }
    ...
    function _mixColumn(bytes4 column) internal pure returns (bytes4) {
        bytes1 b0 = MUL_BY_2[uint8(column[0])] ^ MUL_BY_3[uint8(column[1])] ^ column[2] ^ column[3];
        bytes1 b1 = column[0] ^ MUL_BY_2[uint8(column[1])] ^ MUL_BY_3[uint8(column[2])] ^ column[3];
        bytes1 b2 = column[0] ^ column[1] ^ MUL_BY_2[uint8(column[2])] ^ MUL_BY_3[uint8(column[3])];
        bytes1 b3 = MUL_BY_3[uint8(column[0])] ^ column[1] ^ column[2] ^ MUL_BY_2[uint8(column[3])];
        return _bytes1ToBytes4(b0, b1, b2, b3);
    }
    ...
    function addRoundKey(AESState memory state, bytes16 round_key)
        public
        pure
        returns (AESState memory)
    {
        ...
        state.col0 = state.col0 ^ round_key_col_0;
        state.col1 = state.col1 ^ round_key_col_1;
        state.col2 = state.col2 ^ round_key_col_2;
        state.col3 = state.col3 ^ round_key_col_3;
        return state;
    }
    ...
    /// @return The bytes4
    function _bytes1ToBytes4(bytes1 a, bytes1 b, bytes1 c, bytes1 d)
        internal
        pure
        returns (bytes4)
    {
        return bytes4(
            (uint32(uint8(a)) << 24) | (uint32(uint8(b)) << 16) | (uint32(uint8(c)) << 8)
                | uint32(uint8(d))
        );
    }

    /// @notice Convert 4 bytes4 to a bytes16
    /// @param b0 The first byte4
    /// @param b1 The second byte4
    /// @param b2 The third byte4
    /// @param b3 The fourth byte4
    /// @return The bytes16
    function _bytes4ToBytes16(bytes4 b0, bytes4 b1, bytes4 b2, bytes4 b3)
        public
        pure
        returns (bytes16)
    {
        return bytes16(
            (uint128(uint32(b0)) << 96) | (uint128(uint32(b1)) << 64) | (uint128(uint32(b2)) << 32)
                | uint128(uint32(b3))
        );
    }
}
